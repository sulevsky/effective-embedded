...
impl<'a, I2C: I2c> Sensor<'a, I2C> {
    ...
    pub fn read_sensor(
        ...
        // 1 write
        self.i2c.write(BARO_DEVICE_ADDR, &[0xF4, 0x2E]).unwrap();
        // 2 wait 4.5 millis
        // Timer::after_micros(4500).await;
        // 3 read
        self.i2c
            .write_read(BARO_DEVICE_ADDR, &[0xF6], &mut buf[..2])
            .unwrap();
        ...
        // 3 read
        self.i2c
            .write_read(BARO_DEVICE_ADDR, &[0xF6], &mut buf)
            .unwrap();
        ...
        let x2 =
            (calibration_data.mc as i32 * 2i32.pow(11)) / (x1 as i32 + calibration_data.md as i32);
        let b5 = x1 + x2;
